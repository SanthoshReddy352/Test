...
-- MODIFIED: Add club_name and club_logo_url
...
-- MODIFIED: Allow admins to update their *own* profile. Super admins can manage all.
...
-- NEW: Create storage bucket for club logos
INSERT INTO storage.buckets (id, name, public)
...
-- NEW: Drop club logo policies (for idempotency)
...
-- NEW: Storage policies for club-logos
...
CREATE POLICY "Admins can upload club logos"
    ON storage.objects FOR INSERT
    WITH CHECK (bucket_id = 'club-logos' AND public.get_admin_role() IS NOT NULL);
